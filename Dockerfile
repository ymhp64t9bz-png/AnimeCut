...
# ✂️ AnimeCut Serverless V15.9.3 - NVENC FORÇADO
# CORREÇÕES: GPU OBRIGATÓRIA, 4 tentativas NVENC antes de CPU
# FORÇA REBUILD LIMPO - SEM CACHE
FROM runpod/pytorch:2.2.1-py3.10-cuda12.1.1-devel-ubuntu22.04

# ==================== FORÇA REBUILD SEM CACHE ====================
ARG FORCE_REBUILD=5
ARG BUILD_TIMESTAMP=20251222_1700_V15_9_3_NVENC_FORCED
...
# ==================== CACHE BUSTER ====================
ARG CACHEBUST=20251222_1700_V15_9_3_GPU_FORCED
RUN echo "Build timestamp: ${CACHEBUST}" > /BUILD_INFO && \
    echo "V15.9.3 - NVENC FORÇADO (GPU obrigatória)" >> /BUILD_INFO && \
    echo "Build ID: $(cat /proc/sys/kernel/random/uuid 2>/dev/null || echo $RANDOM)" >> /BUILD_INFO

WORKDIR /app

# Variáveis de Ambiente
ENV BUILD_VERSION="15.9.3"
ENV BUILD_DATE="2025-12-22T17:00:00Z"
...
ARG HANDLER_NOCACHE=15.9.3_20251222_1700_NVENC_FORCED
RUN echo "Handler rebuild: ${HANDLER_NOCACHE} - $(date)" > /tmp/handler_build.txt

COPY handler.py .

RUN echo "=== BUILD COMPLETO v15.9.3 ===" && \
    echo "Handler timestamp: $(date -Iseconds)" && \
    echo "Correções: NVENC FORÇADO, GPU obrigatória, 4 tentativas" && \
