...
# ✂️ AnimeCut Serverless V15.9.5 - ENCODING SIMPLIFICADO
# CORREÇÕES: Fluxo único robusto, logs detalhados
# FORÇA REBUILD LIMPO - SEM CACHE
FROM runpod/pytorch:2.2.1-py3.10-cuda12.1.1-devel-ubuntu22.04

# ==================== FORÇA REBUILD SEM CACHE ====================
ARG FORCE_REBUILD=7
ARG BUILD_TIMESTAMP=20251222_2000_V15_9_5_SIMPLIFIED
...
# ==================== CACHE BUSTER ====================
ARG CACHEBUST=20251222_2000_V15_9_5_ROBUST
RUN echo "Build timestamp: ${CACHEBUST}" > /BUILD_INFO && \
    echo "V15.9.5 - Encoding simplificado e robusto" >> /BUILD_INFO && \
    echo "Build ID: $(cat /proc/sys/kernel/random/uuid 2>/dev/null || echo $RANDOM)" >> /BUILD_INFO

WORKDIR /app

# Variáveis de Ambiente
ENV BUILD_VERSION="15.9.5"
ENV BUILD_DATE="2025-12-22T20:00:00Z"
...
ARG HANDLER_NOCACHE=15.9.5_20251222_2000_SIMPLIFIED
RUN echo "Handler rebuild: ${HANDLER_NOCACHE} - $(date)" > /tmp/handler_build.txt

COPY handler.py .

RUN echo "=== BUILD COMPLETO v15.9.5 ===" && \
    echo "Handler timestamp: $(date -Iseconds)" && \
    echo "Correções: Encoding simplificado, logs detalhados" && \
