...
# ✂️ AnimeCut Serverless V15.9.2 - FIX TRAVAMENTO
# CORREÇÕES: FFmpeg direto (sem MoviePy que trava), Timeout 180s
# FORÇA REBUILD LIMPO - SEM CACHE
FROM runpod/pytorch:2.2.1-py3.10-cuda12.1.1-devel-ubuntu22.04

# ==================== FORÇA REBUILD SEM CACHE ====================
ARG FORCE_REBUILD=4
ARG BUILD_TIMESTAMP=20251222_1630_V15_9_2_FFMPEG_DIRETO
...
# ==================== CACHE BUSTER ====================
ARG CACHEBUST=20251222_1630_V15_9_2_NO_HANG
RUN echo "Build timestamp: ${CACHEBUST}" > /BUILD_INFO && \
    echo "V15.9.2 - FIX: FFmpeg direto, Timeout 180s" >> /BUILD_INFO && \
    echo "Build ID: $(cat /proc/sys/kernel/random/uuid 2>/dev/null || echo $RANDOM)" >> /BUILD_INFO

WORKDIR /app

# Variáveis de Ambiente
ENV BUILD_VERSION="15.9.2"
ENV BUILD_DATE="2025-12-22T16:30:00Z"
...
ARG HANDLER_NOCACHE=15.9.2_20251222_1630_FFMPEG_DIRETO
RUN echo "Handler rebuild: ${HANDLER_NOCACHE} - $(date)" > /tmp/handler_build.txt

COPY handler.py .

RUN echo "=== BUILD COMPLETO v15.9.2 ===" && \
    echo "Handler timestamp: $(date -Iseconds)" && \
    echo "Correções: FFmpeg direto, Timeout 180s, MAX 2 linhas" && \
