...
# -*- coding: utf-8 -*-
"""
AnimeCut Serverless v15.9.2 FIX TRAVAMENTO
BUILD: 2025-12-22 16:30 - FFMPEG DIRETO (SEM MOVIEPY)
Stack: Whisper V3 Turbo, YOLOv8, DeepFilterNet, FFmpeg NVENC/libx264
CORREÇÕES: 
- ENCODING: FFmpeg DIRETO com overlay (não trava como MoviePy)
- TIMEOUT: 180s por corte (não fica infinito)
- Título: MAX 2 linhas com auto-redimensionamento
- Layout: Background → Vídeo → Título (ordem correta)
"""
...
def processar_corte_gpu(video_path: str, cut_data: Dict, num: int, config: Dict) -> str:
    ...
    try:
        ...
        # ==================== ENCODING v15.9.2 - FFMPEG COM DRAWTEXT ====================
        # PROBLEMA v15.9.1: MoviePy trava em cortes longos (100s+)
        # SOLUÇÃO: FFmpeg direto com drawtext (sem PNG intermediário)
        
        logger.info("=" * 60)
        logger.info("[ENCODING v15.9.2] FFMPEG DIRETO + DRAWTEXT")
        logger.info("=" * 60)
        ...
        # ==================== MÉTODO 1: FFMPEG COM OVERLAY DE IMAGENS ====================
        try:
            logger.info("[ENCODING] Método 1: FFmpeg com overlay")
            ...
            if title_clip:
                try:
                    ...
                    # Cria imagem 1080x1920 com título na posição correta
                    ...
                    full_img.paste(title_img, (x_pos, title_y), title_img)
            ...
            # Ordem dos inputs: [0]=vídeo, [1]=background, [2]=título
            
            if nvenc_available:
                encoder = ['-c:v', 'h264_nvenc', '-preset', 'p4', '-b:v', '6M']
            else:
                encoder = ['-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23']
            
            # Filter complex baseado no que temos disponível
            if bg_png and bg_png.exists() and title_png and title_png.exists():
                # Tem background E título
                filter_complex = (
            ...
            logger.info(f"[FFMPEG] Executando... (timeout: 180s)")
            ...
            if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 50000:
                ...
                logger.info("=" * 60)
                logger.info(f"[✓ SUCCESS] Corte {num} - FFmpeg overlay!")
                ...
                logger.warning(f"[FFMPEG NVENC] Falhou: {err_msg}")
                
                # Se NVENC falhou, tenta com libx264
                if nvenc_available:
                    logger.info("[FFMPEG] Tentando com libx264...")
                    
                    # Recria PNGs se foram deletados
                    if bg_clip and (not bg_png or not bg_png.exists()):
                        try:
                            bg_png = TEMP_DIR / f"bg_{num}_{uuid.uuid4().hex[:6]}.png"
                            bg_frame = bg_clip.get_frame(0)
                            if bg_frame.dtype != np.uint8:
                    ...
                    if title_clip and (not title_png or not title_png.exists()):
                        try:
                            title_png = TEMP_DIR / f"title_{num}_{uuid.uuid4().hex[:6]}.png"
                            title_style = config.get("titleStyle", {})
                            pos_vertical = title_style.get("verticalPosition", 25) / 100.0
                    ...
                    # Reconstroi inputs e filter_complex
                    encoder = ['-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23']
                    ...
                    try:
                        ...
                        if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 50000:
                            ...
                            logger.info("=" * 60)
                            logger.info(f"[✓ SUCCESS] Corte {num} - FFmpeg libx264!")
                            logger.info(f"    Tempo: {encode_time:.1f}s")
                            logger.info("=" * 60)
                    except Exception as e:
                        logger.warning(f"[FFMPEG libx264] Também falhou: {str(e)[:100]}")
            ...
            logger.warning("[FFMPEG] Timeout (180s)!")
        except Exception as e:
        ...
        if not encoding_success:
            try:
                logger.info("[ENCODING] Método 2: FFmpeg simples (sem título/bg)")
                ...
                if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 10000:
                    ...
                    logger.warning(f"[FFMPEG SIMPLES] Falhou: {proc.stderr[:200] if proc.stderr else ''}")
                    
            except subprocess.TimeoutExpired:
                logger.warning("[FFMPEG SIMPLES] Timeout!")
            except Exception as e:
                logger.warning(f"[FFMPEG SIMPLES] Erro: {str(e)[:100]}")
        
        # ==================== MÉTODO 3: CORTE BRUTO (ÚLTIMO RECURSO) ====================
        if not encoding_success:
            logger.warning("[ENCODING] Método 3: Corte bruto (sem composição)")
...
if __name__ == "__main__":
    try:
        ...
        print("\n" + "="*70)
        print("╔═══════════════════════════════════════════════════════════════════╗")
        print("║   ANIMECUT SERVERLESS v15.9.2 - BUILD 2025-12-22 16:30           ║")
        print("║   🔧 FIX: FFmpeg direto (sem travamento) + Timeout 180s          ║")
        print("╚═══════════════════════════════════════════════════════════════════╝")
        print("Correções v15.9.2:")
        print("  ✓ ENCODING: FFmpeg DIRETO com overlay (não trava)")
        print("  ✓ TIMEOUT: 180s por corte (não fica infinito)")
        print("  ✓ TÍTULO: Máximo 2 linhas (fonte diminui automaticamente)")
        print("  ✓ LAYOUT: Background → Vídeo → Título (ordem correta)")
        print("  ✓ FALLBACK: FFmpeg simples → Corte bruto")
