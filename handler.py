...
# -*- coding: utf-8 -*-
"""
AnimeCut Serverless v15.9.4 FIX TRANSPARÊNCIA TÍTULO
BUILD: 2025-12-22 17:30 - PNG COM ALPHA CORRETO
Stack: Whisper V3 Turbo, YOLOv8, DeepFilterNet, FFmpeg NVENC
CORREÇÕES: 
- TÍTULO PNG: Criado diretamente com PIL (preserva transparência)
- NÃO usa MoviePy para título (perdia canal alpha)
- NVENC FORÇADO: GPU como método primário
- Background visível atrás do título
"""
    ...
    try:
        ...
        if nvenc_available:
            try:
                ...
                # 2. Salva título como PNG COM TRANSPARÊNCIA
                title_png = None
                if title and config.get("generateTitles", True):
                    try:
                        ...
                        # Usa função que preserva transparência (NÃO usa MoviePy)
                        ...
                        if result and title_png_path.exists():
                            ...
                            logger.info(f"[TITULO] PNG transparente criado")
                        else:
                            logger.warning("[TITULO] Falha ao criar PNG transparente")
        ...
        if not encoding_success and nvenc_available:
            try:
                ...
                # Recria título PNG com transparência
                title_png = None
                if title and config.get("generateTitles", True):
                    try:
                        title_png_path = TEMP_DIR / f"title_{num}_{uuid.uuid4().hex[:6]}.png"
...
if __name__ == "__main__":
    try:
        ...
        print("\n" + "="*70)
        print("╔═══════════════════════════════════════════════════════════════════╗")
        print("║   ANIMECUT SERVERLESS v15.9.4 - BUILD 2025-12-22 17:30           ║")
        print("║   🔧 FIX: Título PNG com transparência (background visível)      ║")
        print("╚═══════════════════════════════════════════════════════════════════╝")
        print("Correções v15.9.4:")
        print("  ✓ TÍTULO: PNG criado com PIL (preserva transparência)")
        print("  ✓ BACKGROUND: Visível atrás do título (não mais preto)")
        print("  ✓ NVENC: GPU como método primário")
        print("  ✓ LAYOUT: Background → Vídeo → Título (ordem correta)")
