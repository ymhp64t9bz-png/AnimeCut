...
# -*- coding: utf-8 -*-
"""
AnimeCut Serverless v15.9.3 NVENC FORÇADO
BUILD: 2025-12-22 17:00 - GPU OBRIGATÓRIA
Stack: Whisper V3 Turbo, YOLOv8, DeepFilterNet, FFmpeg NVENC
CORREÇÕES: 
- NVENC FORÇADO: GPU como método PRIMÁRIO (não CPU!)
- HWACCEL: Decodificação CUDA + Encoding NVENC
- 4 tentativas com GPU antes de cair para CPU
- Timeout 120s por método (não trava)
"""
...
def processar_corte_gpu(video_path: str, cut_data: Dict, num: int, config: Dict) -> str:
    ...
    try:
        ...
        # ==================== ENCODING v15.9.3 - NVENC FORÇADO ====================
        # PROBLEMA: Sistema caindo para CPU (35+ min) mesmo com GPU disponível
        # SOLUÇÃO: FORÇAR uso de NVENC com hwaccel para decode + encode
        
        logger.info("=" * 60)
        logger.info("[ENCODING v15.9.3] NVENC FORÇADO - GPU OBRIGATÓRIA")
        logger.info("=" * 60)
        ...
        # Verifica NVENC - DEVE estar disponível
        ...
        try:
            ...
            if nvenc_available:
                logger.info("[GPU] ✓ NVENC DISPONÍVEL - USANDO GPU!")
            else:
                logger.error("[GPU] ✗ NVENC NÃO DISPONÍVEL - PROBLEMA GRAVE!")
        except Exception as e:
            logger.error(f"[GPU] Erro ao verificar NVENC: {e}")
        
        # Configurações de vídeo
        w, h = clip.w, clip.h
        clip_aspect = w / h
        
        # Calcula tamanho do vídeo para letterbox
        ...
        # ==================== MÉTODO 1: NVENC PURO (GPU TOTAL) ====================
        if nvenc_available:
            try:
                logger.info("[ENCODING] Método 1: NVENC PURO (GPU decode + encode)")
                
                # 1. Salva background como PNG
                bg_png = None
                if bg_clip:
                    try:
                        ...
                        Image.fromarray(bg_frame).save(str(bg_png), 'PNG')
                        logger.info(f"[BG] PNG salvo: {bg_png}")
                    except Exception as e:
                        logger.warning(f"[BG] Erro: {e}")
                        bg_png = None
                
                # 2. Salva título como PNG
                title_png = None
                if title_clip:
                    try:
                        ...
                        title_y = int(target_h * pos_vertical)
                        title_y = max(0, min(target_h - title_h, title_y))
                        
                        full_img = Image.new('RGBA', (target_w, target_h), (0, 0, 0, 0))
                        title_img = Image.fromarray(title_frame)
                        if title_img.mode != 'RGBA':
                            title_img = title_img.convert('RGBA')
                        full_img.paste(title_img, ((target_w - title_w) // 2, title_y), title_img)
                        full_img.save(str(title_png), 'PNG')
                        logger.info(f"[TITULO] PNG salvo em Y={title_y}")
                    except Exception as e:
                        logger.warning(f"[TITULO] Erro PNG: {e}")
                        title_png = None
                
                # 3. Monta comando FFmpeg com NVENC
                # ENCODER NVENC COM CONFIGURAÇÃO ROBUSTA
                encoder = [
                    '-c:v', 'h264_nvenc',
                    '-preset', 'p4',           # Preset balanceado (p1=rápido, p7=qualidade)
                    '-tune', 'hq',             # Tune para alta qualidade
                    '-rc', 'vbr',              # Variable bitrate
                    '-cq', '23',               # Qualidade constante (similar a CRF)
                    '-b:v', '6M',              # Bitrate target
                    '-maxrate', '10M',         # Máximo
                    '-bufsize', '12M',         # Buffer
                    '-profile:v', 'high',      # Perfil H.264
                    '-level', '4.1',           # Nível de compatibilidade
                    '-spatial-aq', '1',        # Adaptive quantization espacial
                    '-temporal-aq', '1',       # Adaptive quantization temporal
                    '-rc-lookahead', '20',     # Lookahead frames
                ]
                
                # Filter complex baseado no que temos
                if bg_png and bg_png.exists() and title_png and title_png.exists():
                    filter_complex = (
                    ...
                    inputs = [
                        ...
                        '-ss', str(start), '-t', str(cut_duration), '-i', video_path,
                        '-loop', '1', '-t', str(cut_duration), '-i', str(bg_png),
                        '-loop', '1', '-t', str(cut_duration), '-i', str(title_png)
                    ]
                    map_stream = '[final]'
                    
                elif bg_png and bg_png.exists():
                    filter_complex = (
                        f"[1:v]scale={target_w}:{target_h}[bg];"
                        f"[0:v]scale={vid_w}:{vid_h}[vid];"
                ...
                elif title_png and title_png.exists():
                    filter_complex = (
                ...
                logger.info(f"[NVENC] Executando com GPU... (timeout: 120s)")
                
                proc = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
                
                # Limpa PNGs
                for f in [bg_png, title_png]:
                    if f and f.exists():
                        try: f.unlink()
                        except: pass
                ...
                if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 50000:
                    ...
                    logger.info(f"    Velocidade: {speed:.2f}x realtime (GPU)")
                    logger.info("=" * 60)
                else:
                    err_msg = proc.stderr[-1000:] if proc.stderr else 'sem detalhes'
                    logger.warning(f"[NVENC] Falhou com hwaccel: {err_msg[:300]}")
                    
            except subprocess.TimeoutExpired:
                logger.warning("[NVENC] Timeout (120s)!")
            except Exception as e:
                logger.warning(f"[NVENC] Erro: {str(e)[:200]}")
        
        # ==================== MÉTODO 2: NVENC SEM HWACCEL ====================
        if not encoding_success and nvenc_available:
            try:
                logger.info("[ENCODING] Método 2: NVENC sem hwaccel (só encode GPU)")
                
                # Recria PNGs se necessário
                bg_png = None
                if bg_clip:
                    try:
                        bg_png = TEMP_DIR / f"bg_{num}_{uuid.uuid4().hex[:6]}.png"
                ...
                if title_clip:
                    try:
                        ...
                        if title_frame.dtype != np.uint8:
                            title_frame = np.clip(title_frame, 0, 255).astype(np.uint8)
                        title_h, title_w = title_frame.shape[0], title_frame.shape[1]
                        title_y = max(0, min(target_h - title_h, int(target_h * pos_vertical)))
                        full_img = Image.new('RGBA', (target_w, target_h), (0, 0, 0, 0))
                        title_img = Image.fromarray(title_frame)
                        if title_img.mode != 'RGBA':
                            title_img = title_img.convert('RGBA')
                        full_img.paste(title_img, ((target_w - title_w) // 2, title_y), title_img)
                        full_img.save(str(title_png), 'PNG')
                    except: title_png = None
                
                # NVENC simples (sem hwaccel no decode)
                ...
                logger.info(f"[NVENC] Executando sem hwaccel... (timeout: 120s)")
                ...
                if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 50000:
                    ...
                    logger.info("=" * 60)
                    logger.info(f"[✓ SUCCESS] Corte {num} - NVENC (sem hwaccel)!")
                    logger.info(f"    Tempo: {encode_time:.1f}s | Velocidade: {speed:.2f}x")
                    logger.info("=" * 60)
                else:
                    err_msg = proc.stderr[-500:] if proc.stderr else ''
                    logger.warning(f"[NVENC] Também falhou: {err_msg[:200]}")
                    
            except subprocess.TimeoutExpired:
                logger.warning("[NVENC] Timeout!")
            except Exception as e:
                logger.warning(f"[NVENC] Erro: {str(e)[:100]}")
        
        # ==================== MÉTODO 3: NVENC SIMPLES (SEM OVERLAY) ====================
        if not encoding_success and nvenc_available:
            try:
                logger.info("[ENCODING] Método 3: NVENC simples (sem overlay)")
                ...
                if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 10000:
                    ...
                    logger.info("=" * 60)
                    logger.info(f"[✓ SUCCESS] Corte {num} - NVENC simples")
                    logger.info(f"    ⚠ SEM título/background (fallback GPU)")
                    logger.info("=" * 60)
                else:
                    logger.warning(f"[NVENC SIMPLES] Falhou: {proc.stderr[-200:] if proc.stderr else ''}")
                    
            except Exception as e:
                logger.warning(f"[NVENC SIMPLES] Erro: {str(e)[:100]}")
        
        # ==================== MÉTODO 4: CPU APENAS SE GPU FALHAR COMPLETAMENTE ====================
        if not encoding_success:
            logger.error("[ENCODING] ⚠ ATENÇÃO: GPU falhou, usando CPU (LENTO!)")
            ...
            try:
                ...
                if proc.returncode == 0 and output_path.exists() and output_path.stat().st_size > 10000:
                    ...
                    logger.info("=" * 60)
                    logger.info(f"[✓ SUCCESS] Corte {num} - CPU (LENTO)")
        ...
        # ==================== MÉTODO 5: CORTE BRUTO (ÚLTIMO RECURSO) ====================
        if not encoding_success:
            logger.warning("[ENCODING] Método 5: Corte bruto (sem composição)")
...
if __name__ == "__main__":
    try:
        ...
        print("\n" + "="*70)
        print("╔═══════════════════════════════════════════════════════════════════╗")
        print("║   ANIMECUT SERVERLESS v15.9.3 - BUILD 2025-12-22 17:00           ║")
        print("║   🔧 NVENC FORÇADO - GPU OBRIGATÓRIA (NÃO USA CPU!)              ║")
        print("╚═══════════════════════════════════════════════════════════════════╝")
        print("Correções v15.9.3:")
        print("  ✓ NVENC: GPU como método PRIMÁRIO (4 tentativas)")
        print("  ✓ HWACCEL: Decode CUDA + Encode NVENC")
        print("  ✓ TIMEOUT: 120s por método (não trava)")
        print("  ✓ TÍTULO: MAX 2 linhas, posição correta")
        print("  ✓ FALLBACK: CPU só se GPU falhar 4 vezes")
